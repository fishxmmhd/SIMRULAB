    ...
    $.fn.formToWizard = function( options, cmdParam1 ) {
        ...
        function createNextButton(i) {
            ...
            $("#" + stepName + "Next").bind( "click", function(e) {
                ...
                var tanggal_mulai = tanggal + ' ' + document.getElementById('waktu_mulai').value + ':00'
                var tanggal_selesai = tanggal + ' ' + document.getElementById('waktu_selesai').value + ':00'
                var ruangan = document.getElementById('ruanganinput').value;
                var url = "/pemakaian/cek/"+ tanggal + "/" + tanggal_mulai + "/" + tanggal_selesai + "/" + ruangan
                $.ajax({url: url}).done(function(result){
                  if(result.status){
                    stepvalidated = result.status;
                    $("#" + stepName).hide();
                    $("#step" + (i + 1)).show();
                    //if (i + 2 == count)
                    selectStep(i + 1);
                    ...
                    console.log('bege')
                    document.getElementById('alertError').innerHTML = '<div class="alert alert-danger">Penggunaan untuk waktu dan ruangan ini tidak tersedia!</div>'
                    return false;
                }
                if(result.libur){
                    document.getElementById('biayaTambahan').setAttribute('class','form-group')
                }else{
                    document.getElementById('biayaTambahan').setAttribute('class','form-group hidden')
                }
            });
            });
        }
